...
chemin1="xxxxx" #for now, needs to contain all the R scripts called in this one and all the files with row data from the experiment
setwd(chemin1)
...
data_files_csv = list()
data_files_tsv = list()
data_files_tsv <- list.files("xxxx/data_files_tsv")  
data_files_csv <- list.files("xxxx/data_files_csv")
...
#-----------end-------------
k=k+1
rm(list=ls()[! ls() %in% c("data_files_csv","data_files_tsv","dataPosImg",
                           "dataNegImg","dataNeuImg","dataPosFilm","dataNeuFilm","dataNegFilm","dataHR","k")])
